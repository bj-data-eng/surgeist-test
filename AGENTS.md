...
Use this guide for automated work in the `surgeist-test` crate repo.
...
This repo owns the `surgeist-test` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It
may link this repo as a submodule under `crates/surgeist-test` if the top-level
coordinator chooses to include shared test infrastructure in root integration
checks.

This crate repo owns reusable test harnesses, fixture metadata, generated test
artifacts when they are intentionally moved here, crate-local plans,
...
Production crates should not depend on surgeist-test without coordinator approval.
```
...
Prefer existing crate-local dependencies. New dependencies must justify why they
belong at this layer, whether they affect MSRV, licenses, advisories, binary
size, optional features, or dependency cycles.
...
2. Confirm the work belongs in `surgeist-test`.
